...
\begin{frame}{Okolje \texttt{align} in \texttt{align*}}
	...
	\begin{align*}
	(a+b)^n &= \ldots \\ 
	\only<3>{&= (a+b) (a+b) \dots (a+b) \\}
	\onslide<2-3>{&= a^n + n a^{n-1} b + \dots + \binom{n}{k} a^{n-k} b^k + \dots + n a b^{n-1} + b^n \\}
...
f(x, y) =
\begin{cases} 
	\frac{3x^2y-y^3}{x^2+y^2} \quad & \text{za } (x, y) \neq (0, 0), \\
    a \quad & \text{za } (x, y) = (0, 0).
\end{cases}
\]
